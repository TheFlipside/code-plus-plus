...
define_class!(
    ...
    unsafe impl NSMenuDelegate for Actions {
        ...
        ///
        /// Four menus need it. The Window menu's buffer list changes
        ...
        ///
        /// The Plugins menu is here for a different reason: this is the
        /// **lazy-load trigger**. Nothing is `dlopen`ed until the menu is
        /// first opened (DESIGN.md §6.4), so the rebuild both maps the
        /// plugins and builds their items from what came back.
        ///
        /// Each menu is identified by its **title** ([`is_file_menu`],
        /// [`is_window_menu`], [`is_language_menu`], [`is_plugins_menu`]),
        /// so one delegate serves all four and any future menu with the
        /// same need.
        ...
        fn menu_needs_update(&self, menu: &NSMenu) {
            crate::at_callback_boundary("menu:needsUpdate", (), || {
                ...
                } else if is_plugins_menu(menu) {
                    crate::plugin::ensure_loaded_and_rebuild(menu, self);
                }
            });
        }
    }
    ...
    impl Actions {
        ...
        /// Run one plugin's menu command. Carries the host-allocated
        /// `FuncItem.cmd_id` in its tag — an **id**, not an index, so a
        /// rebuild between opening the menu and clicking it resolves to
        /// "gone" rather than to a different command.
        #[unsafe(method(codeppPluginCommand:))]
        fn plugin_command(&self, sender: Option<&NSMenuItem>) {
            crate::at_callback_boundary("menu:pluginCommand", (), || {
                if let Some(sender) = sender {
                    crate::plugin::on_plugin_command(sender.tag() as i32);
                }
            });
        }
        ...
        fn plugin_manager(&self, _sender: Option<&NSObject>) {
            crate::at_callback_boundary(
                "menu:pluginManager",
                (),
                crate::plugin::show_plugin_manager,
            );
        }

        #[unsafe(method(codeppOpenPluginFolder:))]
        fn open_plugin_folder(&self, _sender: Option<&NSObject>) {
            crate::at_callback_boundary(
                "menu:openPluginFolder",
                (),
                crate::plugin::open_plugin_folder,
            );
        }
...
pub fn install(app: &NSApplication, actions: &Actions, mtm: MainThreadMarker) -> Retained<NSMenu> {
    ...
    main_menu.addItem(&build_plugins_menu(actions, mtm));
...
/// Title identifying the Plugins menu to the shared menu delegate.
/// Same title-match reasoning as [`WINDOW_MENU_TITLE`].
const PLUGINS_MENU_TITLE: &str = "Plugins";
...
/// The Plugins menu.
///
/// Built **empty** but for a placeholder: its whole contents are
/// regenerated by [`crate::plugin::ensure_loaded_and_rebuild`] on the
/// first open, which is also when the plugin DLLs are actually mapped
/// (DESIGN.md §6.4 — nothing is `dlopen`ed at startup). So the
/// placeholder is what a user sees only if no plugin is installed.
///
/// Notepad++ puts this menu between Run and Window; Code++ has neither
/// Settings, Tools, Macro nor Run yet on this backend, so it lands
/// directly after Language — i.e. after the last built-in menu and
/// before Window and `?`. `ui_gtk` places it after Language too, though
/// the comparison stops there: that backend has no Window menu.
fn build_plugins_menu(actions: &Actions, mtm: MainThreadMarker) -> Retained<NSMenuItem> {
    let (item, menu) = submenu(PLUGINS_MENU_TITLE, mtm);
    add_disabled(&menu, mtm, "No plugins loaded");
    // SAFETY: `Actions` implements `NSMenuDelegate`, and AppKit's
    // delegate reference is weak — the window state owns `actions` for
    // the process lifetime, so it cannot dangle.
    menu.setDelegate(Some(ProtocolObject::from_ref(actions)));
    item
}
...
pub(crate) fn add_disabled(menu: &NSMenu, mtm: MainThreadMarker, title: &str) {
    let item = NSMenuItem::new(mtm);
    item.setTitle(&NSString::from_str(title));
    menu.addItem(&item);
}
