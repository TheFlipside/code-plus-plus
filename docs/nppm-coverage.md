...
| `NPPM_MODELESSDIALOG` | ✅ | v3 | wparam: `MODELESSDIALOGADD` (0) or `MODELESSDIALOGREMOVE` (1); lparam: HWND of the plugin's dialog. Returns lparam on success, 0 on bad args. The host stores plugin dialogs on `WindowState.plugin_modeless_dialogs: Vec<HWND>`; the message pump clones the Vec each iteration (cheap — typically 0..2 entries) and calls `IsDialogMessageW` against each, after the host's own Find/Replace dialog so a plugin can't shadow host keyboard handling. Each per-dialog `IsDialogMessageW` is preceded by an `IsWindow` guard so a plugin that forgot to REMOVE before destroying the HWND gets a clean miss instead of UB on a freed handle. ADD is idempotent (de-dup before push so the pump doesn't fire `IsDialogMessageW` twice per keystroke). Same kernel-mode-pointer guard (`lparam <= 0`) as the allocator pair and shortcut messages. |
...
| `NPPM_CREATESCINTILLAHANDLE` | ✅ | v3 | wparam unused; lparam: parent HWND. Creates a fresh Scintilla control as a child of the plugin's HWND with `WS_CHILD | WS_CLIPCHILDREN | WS_VSCROLL | WS_HSCROLL`, initialised to `SCI_SETCODEPAGE = SC_CP_UTF8` so plugin-owned views inherit the host's UTF-8-internal invariant. Plugin owns the new HWND lifetime (must `DestroyWindow` before the parent dies). Negative / NULL lparam rejected up front (kernel-mode-pointer guard); dead parent HWND rejected via `IsWindow`. Returns the new HWND as `isize`, or 0 on failure. |
| `NPPM_DESTROYSCINTILLAHANDLE` | ⏸ | — | Deprecated upstream; no-op. The expected teardown for a HWND from `NPPM_CREATESCINTILLAHANDLE` is the plugin's own `DestroyWindow` call — same as N++ since this message was deprecated. A plugin that relies on the host to clean up via this message will leak the control until the parent dies. |
| `NPPM_GETNBUSERLANG` | ✅ | v3 | Returns 0. Code++ does not yet implement user-defined languages (no UDL menu, no XML parser, no runtime registry — all Phase 4+ polish). The honest count answers the contract; plugins gating on `if (NPPM_GETNBUSERLANG())` skip their UDL-aware code paths. |
...
| `NPPM_ADDTOOLBARICON` | ✅ | v3 | wparam: cmd id; lparam: `toolbarIcons*` struct (`{ hToolbarBmp, hToolbarIcon }`). Routes through `Win32Ui::add_toolbar_icon`: `TB_GETIMAGELIST` → `ImageList_ReplaceIcon(himl, -1, hicon)` → `TB_ADDBUTTONS` with `idCommand = cmd_id` and `iBitmap = new_index` → `TB_AUTOSIZE` to refresh layout. A click on the new button posts `WM_COMMAND` with the plugin's cmd id, same path Code++'s built-in toolbar buttons use. **Code++ uses `hToolbarIcon` only**; the legacy 16-color `hToolbarBmp` is logged-and-ignored (modern plugins ship the 32-bpp HICON). NULL icon / lparam reject; negative-lparam kernel-mode-pointer guard. Plugin owns the HICON lifetime — `ImageList_ReplaceIcon` internally copies the bits, so the handle can be freed immediately. New `ToolbarIcons` FFI struct in `plugin-host::ffi` with x64/x86 size assertions. |
...
| `NPPM_TRIGGERTABBARCONTEXTMENU` | ✅ | v3 | wparam: view; lparam: tab index (`-1` for active). Returns BOOL — TRUE means "request accepted." Code++ has no tab-bar context menu yet (Phase 4 polish — N++ surfaces Close / Close-Others / Move-to-other-view / Rename / Delete-from-disk entries) so the dispatch is a no-op, but returns TRUE so plugins gating on the return for follow-up work (timer reset, audit log) see "the host received the trigger" rather than a misleading "rejected." When the actual menu lands the return semantics don't change. The trace-log surfaces requests so the future implementation can verify call sites. |
...
| `NPPM_SHOWDOCSWITCHER` / `ISDOCSWITCHERSHOWN` / `DOCSWITCHERDISABLECOLUMN` | ✅ | v3 | Code++ has no doc-switcher panel; the three messages are honest no-ops. `IS*` reports `false`; `SHOW*` is a no-op returning `false` (the previous state — never shown); `DISABLECOLUMN` is a no-op returning 0. Plugins gating on `IS*SHOWN` see the right answer. |
| `NPPM_GETEDITORDEFAULTFOREGROUNDCOLOR` / `...BACKGROUNDCOLOR` | ✅ | v3 | Returns Win32 `COLORREF` (`0x00BBGGRR`) read via `SCI_STYLEGETFORE`/`SCI_STYLEGETBACK` on `STYLE_DEFAULT`. The `as i32` widening is the identity bit pattern. |
| `NPPM_SETSMOOTHFONT` | ✅ | v3 | wparam = `BOOL`. `TRUE` → `SC_EFF_QUALITY_LCD_OPTIMIZED`; `FALSE` → `SC_EFF_QUALITY_NON_ANTIALIASED`. Returns the previous state via a process-wide `SMOOTH_FONT_STATE` shadow (Scintilla has no `SCI_GETFONTQUALITY` peer). |
| `NPPM_SETEDITORBORDEREDGE` | ✅ | v3 | wparam = `BOOL`. Toggles `WS_EX_CLIENTEDGE` on the Scintilla HWND via `GetWindowLongPtrW` / `SetWindowLongPtrW`, then `SetWindowPos(SWP_FRAMECHANGED)` so the chrome repaints. Returns the previous state (read live from the window-style word). No-op when the new value matches the current state. |
| `NPPM_SAVEFILE` | ✅ | v3 | wparam: unused; lparam = TCHAR* path of buffer to save. Routes through `Shell::save_current_to_disk` if the path matches the active tab. Path comparison is byte-level `PathBuf` equality — a plugin that round-trips a path through `NPPM_GETFULLPATHFROMBUFFERID` always sees a successful match; a plugin that synthesises its own path string (different separator, different case) on Windows may see a false-negative `0` return. **Phase 4 limitation (DESIGN.md §7.4):** cross-tab save (saving a background buffer) requires the doc-pointer-swap dance — for now the dispatcher returns 0 with a `tracing::warn!` for known-but-non-active paths. |
| `NPPM_DISABLEAUTOUPDATE` | ✅ | v3 | No-op returning 0. Code++ has no auto-update mechanism; the message exists in the dispatch table so plugins compiled against the latest header link without surfacing an "unhandled NPPM_*" warning at every call. |
| `NPPM_GETLINENUMBERWIDTHMODE` | ✅ | v3 | Returns `LINENUMWIDTH_DYNAMIC` (0). Code++'s line-number margin uses dynamic width everywhere; the constant-mode pin is a Phase 4 polish item. |
| `NPPM_SETLINENUMBERWIDTHMODE` | ✅ | v3 | wparam: `LINENUMWIDTH_DYNAMIC` or `LINENUMWIDTH_CONSTANT`. Accepts both values (returns 1) so plugins' set-then-verify round-trips behave; `CONSTANT` mode currently does not pin the visible margin width — the `GET` peer reports `DYNAMIC` until Phase 4 polish lands. |
...
| `NPPM_GETSHORTCUTBYCMDID` | ✅ | v3 | wparam: cmd id; lparam: `ShortcutKey*` OUT (4 bytes — the `is_ctrl`/`is_alt`/`is_shift`/`key` quartet). Returns 1 on hit, 0 on miss / NULL lparam / negative lparam (kernel-mode-pointer guard, same shape as `NPPM_MSGTOPLUGIN`). Live introspection via `CopyAcceleratorTableW` against the HACCEL stored on `WindowState.accel_handle` — the same handle the message pump consults via `TranslateAcceleratorW`, so the lookup always reflects the current binding even after `NPPM_REMOVESHORTCUTBYCMDID` mutations. |
| `NPPM_REMOVESHORTCUTBYCMDID` | ✅ | v3 | wparam: cmd id whose binding to drop. Returns 1 if a binding existed and was removed, 0 otherwise. Win32 has no in-place accelerator-table mutation API; the impl reads the table out via `CopyAcceleratorTableW`, filters out the cmd id, creates a new HACCEL via `CreateAcceleratorTableW`, writes the new handle through `Win32Ui.accel_handle` (a `*mut HACCEL` pointing at `WindowState.accel_handle`), and finally `DestroyAcceleratorTable`s the old handle. Order is write-new-then-destroy-old so the message pump (which reads the slot fresh on every iteration) never sees a freed handle. The new HACCEL is also freed on `WM_DESTROY` so we don't leak on graceful shutdown. The trait method is platform-agnostic — Phase 5 GTK/Cocoa backends mutate their action maps directly without the recreate dance. |
...
| `NPPM_ALLOCATESUPPORTED` | ✅ | v3 | Returns `TRUE` — Code++ supports both `NPPM_ALLOCATECMDID` and `NPPM_ALLOCATEMARKER`. Plugins gating `if (NPPM_ALLOCATESUPPORTED) { … }` take the allocating path. |
| `NPPM_ALLOCATECMDID` | ✅ | v3 | wparam: count of consecutive command IDs requested. lparam: `int*` OUT — the host writes the starting id. Returns 1 on success (plugin uses `start..start+count`), 0 on bad args (`count <= 0`, NULL `lparam`) or pool exhaustion. Pool: `PLUGIN_ALLOC_CMD_BASE..PLUGIN_ALLOC_CMD_LIMIT` (60_000..65_500 — 5500 ids). Distinct from `PLUGIN_CMD_ID_BASE` (50_000) which feeds plugin FuncItem menu commands; the two ranges can't collide. Allocations are durable for the host's lifetime — the upstream contract has no deallocate path. The pool is held in `PluginHost.next_alloc_cmd_id` and bumped atomically; failed allocations leave the counter unchanged so a retry doesn't burn ids. |
| `NPPM_ALLOCATEMARKER` | ✅ | v3 | Same shape as `NPPM_ALLOCATECMDID`. Pool: `PLUGIN_ALLOC_MARKER_BASE..PLUGIN_ALLOC_MARKER_LIMIT` (25..=31 — seven markers, sized by Scintilla's 32-marker cap minus the 0..=23 reserved range and the bookmark slot at 24). Even modest plugins can exhaust the pool — plugins should check the return and fall back to a non-marker path on failure. |
