...
| `NPPM_GETSHORTCUTBYCMDID` | ✅ | v3 | wparam: cmd id; lparam: `ShortcutKey*` OUT (4 bytes — the `is_ctrl`/`is_alt`/`is_shift`/`key` quartet). Returns 1 on hit, 0 on miss / NULL lparam / negative lparam (kernel-mode-pointer guard, same shape as `NPPM_MSGTOPLUGIN`). The Win32 lookup mirrors the accelerator table built in `run()` — hardcoded match because the HACCEL is currently function-local. When `NPPM_REMOVESHORTCUTBYCMDID` lands the lookup will switch to live `CopyAcceleratorTableW` introspection so removed bindings stop appearing here. |
