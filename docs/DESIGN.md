...
- **`EditorHandle` has no lifetime tie to the Scintilla widget it points at — decide the ownership model before Phase 5 m2 lands tabs on GTK.** `EditorHandle` is `Copy` with no `Drop`, holding three raw pointers (DESIGN.md §4.2's direct-call pair plus the native handle). That is fine as-is on Win32, where the `HWND` outlives the handle by construction, and fine in Phase 5 m1, where `ui_gtk::run` keeps exactly one Scintilla widget alive for the whole `gtk::main()` loop via `from_glib_none`'s `g_object_ref_sink` plus the container's own reference. It stops being obviously fine the moment a tab — and its Scintilla widget — can be closed while another code path still holds a copied `EditorHandle` to it: on GTK, closing the tab drops the container's reference and the widget is finalised, leaving a dangling `direct_ptr` that the next `send` would call through. Note the doc-pointer-swap model already used on Win32 (one view, many `SCI_CREATEDOCUMENT` documents) largely sidesteps this by never destroying the view; if `ui_gtk` adopts the same single-view model the problem does not arise, which is the cheapest resolution. Decide explicitly rather than inheriting it — surfaced by the Phase 5 m1 security audit.
...
- **Decide, explicitly, whether m2's 35% memory rise is accepted.** §8 now records PSS going 8.5 MB (m1) → 11.40 MB (m2) → 11.49 MB (m3), with the whole rise attributable to m2 and the tab strip costing +0.8%. That is past §8's own ">20% regresses the constraint" bar, and §8's framing ("non-negotiable", "no exceptions for 'minor' regressions") does not have a "but there was headroom" clause — so the fact that 11.5 MB is only 46% of the 25 MB budget is an argument *for* accepting it, not a reason the rule does not apply. The two honest outcomes are: (a) formally accept, recording the rationale — the likely bulk is the lexer theme table lifted into `codepp_editor` in m2, which is static, read-only, shared between backends and does not scale with what the user opens, so it is a one-off floor rise rather than a leak; or (b) measure where the 2.9 MB actually went before accepting, since "likely the theme table" is inference, not measurement — `/proc/<pid>/smaps` per-mapping attribution or a build with the table feature-gated out would settle it in an afternoon. Do not let this sit: the value of a hard budget is that it is enforced at the boundary, and this is the boundary.

- **`--perf` does not measure Tab, Enter or Backspace, and measuring them needs a different trigger.** Both backends filter to non-control characters, so those three — real editing operations that do repaint — are counted by neither. The naive fix is wrong: none of them repaints *unconditionally* (Backspace at position 0, or any of them on a read-only buffer, returns before invalidating), so opening a measurement on the key press would leave an orphaned sample that the caret-blink timer eventually closes with a fabricated latency — exactly the failure the Escape filter was added to stop. Doing it properly means opening the interval on a subsequent `SCN_MODIFIED` rather than on the key event, which is a different shape from the current key-press trigger and wants doing once, for both backends, alongside the tail characterisation above.

- **Characterise the keystroke-latency tail properly; the p50 is settled, the p99 is not.** `--perf` now measures per keystroke (§8), and the per-character cost is ~0.7 ms — flat across input rates, identical on an empty buffer and a 10 000-line file, and an order of magnitude inside the 5 ms budget. The tail is the open question: p99 measured 2.35 ms at ordinary typing speed but 37.7 ms at a fast typist's rate, and both figures rest on 88 samples, where p99 is simply the worst one or two observations. That is not a tail, it is an anecdote. What is needed is a longer run (tens of thousands of keystrokes, so p99 is backed by hundreds of samples rather than one) driven by something closer to real keyboard input than `xdotool`'s XTEST injection, which does not reliably preserve inter-key spacing and so may be manufacturing part of the backlog it appears to measure. Until then, do not gate a phase on the p99 figure in either direction — neither "we pass" nor "we fail" is supported by the data. Note this item replaced an earlier one asserting that the metric had to be redefined because it measured the compositor; that assertion came from a broken instrument and is retracted in §8.

- **The Win32 Workspace tree renders unsanitized filenames, and one of its context-menu actions executes the file.** *(Security — highest-priority item in this list.)* `insert_tree_folder_children` writes `entry.file_name().to_string_lossy()` straight into `TVITEMW.pszText` with no character filtering, so a filename carrying U+202E RIGHT-TO-LEFT OVERRIDE renders with its tail reversed: a real `invoice\u{202E}fdp.exe` displays as though it were a PDF. That would be a cosmetic spoof on its own, but the per-file context menu offers **Run by system** whenever `extension_has_registered_handler` succeeds for the *real* extension, and that path calls `open_path_in_default_viewer` → `ShellExecuteW(hwnd, "open", <real path>)`. For an executable the `open` verb means *run it*. So a user who opens an attacker-supplied directory as a Workspace (a git clone, an unpacked archive, a network share), sees what looks like a document, and picks "Run by system" expecting a viewer, executes the binary instead. SmartScreen is a partial mitigant for internet-downloaded files and a click-through even when it fires.

  **The naive fix does not work, and this is the part worth reading before starting.** `tree_item_full_path` reconstructs the real filesystem path by walking up the tree and reading each item's *displayed text* back out via `tree_item_text`, joining the components onto the workspace root. Wrapping the name in `sanitize_filename_for_display` at insert time would therefore make the tree try to open a path containing literal U+FFFD characters, which does not exist on disk — every file in the panel would stop opening. Worse, two differently-hostile real names can sanitize to the same displayed string and collide. The real name has to be preserved out of band: an `lParam`-indexed side table or a `HashMap<HTREEITEM, PathBuf>` owned by `WindowState`, with sanitization applied only to what reaches `pszText` and never to what the path reconstruction reads. That is the same "functional value vs. label" distinction now documented at `apply_fif_prefill`, which deliberately does *not* sanitize because its text is round-tripped back as a real search root. **There is already a working precedent for the fix in the same file:** the find-in-files results list writes the *sanitized* string into the ListView cell while `fif_listview_index` holds the real `PathBuf` per row, and `handle_fif_listview_dblclk` opens from that side table rather than from anything parsed back out of the control. Copy that shape rather than inventing one.

  Win32-only today — `ui_gtk` has no workspace tree — but the same trap applies whenever one is ported, so decide the ownership model there before building it, not after. Needs a Windows runner to verify the panel still opens files after the change. Surfaced by the security audit on the Phase 5 display-sanitization consolidation.
...
**Where the growth came from, and an honest note on the 20% rule.** The figure recorded at m1 was 8.5 MB PSS. The table above is 11.5 MB — a 35% rise, which is past the ">20% regression blocks the phase" bar if that bar is read as measurement-over-measurement rather than as headroom against the 25 MB budget (which is still less than half consumed). The rise is **entirely m2's**, and m1's number was never revised when m2 landed, which is how it went unremarked until m3 re-measured. Attribution, both builds measured back-to-back on the same machine with an empty session:

| Build | PSS | `Private_Dirty` |
| --- | --- | --- |
| m1 (`e64f530`) | 8.5 MB | 6.3 MB |
| m2 (`0fda3db`) | 11.40 MB | 6456 kB |
| m3, tab strip (`HEAD`) | 11.49 MB | 6464 kB |

So m2 — the `Shell` wiring, the lifted lexer theme table, the status bar and the menus — cost ~2.9 MB, and the tab strip costs **+89 kB (+0.8%)**. The theme table is the obvious suspect for the bulk of m2's share (85 `*_THEME` rows and ~200 supporting tables became resident on both backends), and it is static, shared, read-only data rather than per-buffer growth, so it does not scale with what the user opens. **This needs an explicit decision, not a note.** §8 opens by calling these constraints non-negotiable and says regressions must be feature-gated or redesigned, "no exceptions for minor regressions" — so recording a 35% rise and moving on is exactly the soft deferral that rule exists to prevent, even though the absolute figure is comfortable. It is tracked as an action in §7.4 rather than left here.
...
**Keystroke latency: measured for the first time on Linux.** `--perf` records each typed character to the redraw that shows it. Measured against a 10 000-line file, varying the input rate, with `xdotool` driving synthetic keystrokes. Collected on a real X11 session with a compositing window manager at 1920x1200/60 Hz — **not** under Xvfb, which has no vsync throttling and where an absent frame-clock floor would prove nothing:

| Input rate | p50 | p99 | samples |
| --- | --- | --- | --- |
| 40 char/s (≈480 wpm — not a human rate) | 10.19 ms | 127.07 ms | 264 |
| 12 char/s (≈144 wpm) | 0.66 ms | 74.19 ms | 132 |
| 7 char/s (≈84 wpm, a fast typist) | 0.72 ms | 37.72 ms | 88 |
| 4 char/s (≈48 wpm, ordinary typing) | 0.70 ms | **2.35 ms** | 88 |

**The per-keystroke cost is ~0.7 ms** and is flat across every rate but the fastest — an order of magnitude inside the 5 ms budget, and identical between an empty buffer and a 10 000-line file, so it does not scale with document size. The §4.2 direct-call path is doing its job.

The tail is queueing, not cost: it grows monotonically with how fast characters are pushed in, which is what a backlog looks like and not what per-keystroke work looks like. At ordinary typing speed the p99 is inside budget; at rates a human cannot sustain it is not.

**Two caveats, both of which mean the tail is not yet properly characterised.** First, `xdotool` injects through XTEST, which does not necessarily preserve the nominal spacing between keystrokes — a "40 char/s" run may deliver tighter bursts than that implies, so the fast rows describe the injector as much as the editor. Second, and more limiting: 88–264 samples puts p99 at the worst one-to-three observations, so those figures are single outliers rather than a stable tail, and a one-off compositor or window-manager hiccup moves them wholesale. Characterising the tail honestly needs a longer run and real keyboard input; that is tracked in §7.4.
...
**Win32 is wired but unmeasured.** `WM_CHAR` in the message pump starts the interval and `SCN_PAINTED` closes it, filtered to the main editor's `hwndFrom` so a plugin's own Scintilla panel cannot consume a pending keystroke. Both platforms filter to characters Scintilla will actually insert — GTK on `!char::is_control` over the keyval's Unicode value, Win32 on the same predicate over the `WM_CHAR` payload, which is the criterion `ScintillaWin.cxx` itself uses to decide whether to insert. So navigation keys, bare modifiers, function keys, Escape, Tab, Enter and Backspace are counted by neither. An earlier revision claimed parity while Win32 in fact filtered on modifier state instead: it counted Tab, Enter and Backspace that GTK never counted, and counted Escape — which in the common case invalidates nothing, so Scintilla never repaints and the press stayed open until an unrelated later paint closed it with a fabricated latency. None of it has been run, because it needs a Windows machine.
