...
**Measured, Phase 5 m1, release build, one empty buffer, x86_64 Linux + GTK 3.24:**

| Figure | Value | Note |
| --- | --- | --- |
| PSS | **8.5 MB** | the constraint above — 34% of budget |
| Private (clean + dirty) | 7.5 MB | of which 6.3 MB dirty |
| `Shared_Clean` | 27.2 MB | GTK / GDK / pango / cairo / glib library text, shared with every GTK process |
| RSS | 34.8 MB | PSS + the shared pages above; recorded for continuity with the old metric |
| `codepp` binary resident | 1.3 MB | all Rust, the gtk3-rs bindings, and statically-linked Scintilla + Lexilla |
...
**Cold start, same build and machine, measured to first draw:** 35.1 / 36.3 / 39.2 / 41.3 / 42.7 ms across five warm-cache runs — roughly half the 80 ms budget. Breakdown of a representative run: `gtk_init` 23.1 ms, `scintilla_new` +1.0 ms, first draw +17.9 ms. The cost is dominated by GTK's own initialisation (theme and CSS parse, icon-theme index, input-method module scan, display connect); Scintilla widget construction is ~1 ms.
