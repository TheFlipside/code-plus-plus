...
| **5 — Linux (GTK 3) and macOS (Cocoa)** | Implement `ui_gtk` and `ui_cocoa` against the same `UiPlatform` trait. Adjust `scintilla-sys` build for the GTK/Cocoa Scintilla backends. **m1 — Linux foundation (landed).** `scintilla-sys` and `editor` un-gated off `#![cfg(target_os = "windows")]` (neither contained platform code beyond its FFI entry points); `build.rs` grew `build_scintilla_gtk` compiling the vendored GTK 3 backend plus Lexilla, with the shared `src/*.cxx` list factored into `scintilla_core_sources`; new `crates/ui_gtk` opens a real window hosting a real Scintilla widget with the §4.2 direct-call pair captured via `EditorHandle::from_gtk_widget`; `app` selects the backend by cargo feature. m1 deliberately stops short of `Shell` — the 48-method `UiPlatform` impl, tabs, dialogs and the plugin host are m2+. Cross-platform plugin loading via `dlopen`/`dlsym`. `app` selects backend at compile time via cargo features (`--features win32` / `--features gtk` / `--features cocoa`), one and only one selected. | `cargo run -p app --features gtk` on Linux: same app, opens, edits, saves, plugin loads (a recompiled `example-hello.so`). Same on macOS with `--features cocoa`. The `core` crate has zero `#[cfg(target_os)]` lines added in this phase — if any appeared, refactor them out before declaring done. |
