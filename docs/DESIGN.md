...
- **Toolbar background colour on the themed Win32 toolbar control.** The Phase 4 m9 toolbar ships with the system-grey backdrop the `ToolbarWindow32` class draws by default. The user asked for plain white. The `SetWindowSubclass` + `WM_ERASEBKGND` override (commit `e75191f`, reverted in `c95e9d8`) didn't land — UxTheme paints over the subclass return on Win10/11 light mode, same wall the active-tab indicator hits below. Viable Phase 5 paths: (a) `TBSTYLE_CUSTOMERASE` + `NM_CUSTOMDRAW` `CDDS_PREERASE` to paint the background ourselves before any theme drawing, returning `CDRF_SKIPDEFAULT`; (b) `SetWindowTheme(toolbar, L"", L"")` to drop the theme entirely (loses the themed button hover/pressed look — not acceptable on its own); (c) full owner-draw via `TBSTYLE_CUSTOMDRAW` so we paint every button cell ourselves alongside the background. Worth attempting (a) alongside the active-tab-indicator owner-draw work below since both fight UxTheme cooperation.

- **Active-tab indicator (orange top edge) on the themed Win32 tab strip.** Notepad++-style "you are editing this buffer" indicator we want is a strong-orange 2–3 px strip along the top edge of the active tab. The `NM_CUSTOMDRAW` approach we tried first is dead-on-arrival on modern Windows: the visual-style theme engine (UxTheme) draws the tab strip itself and largely ignores the custom-draw callbacks, so our overlay never lands. The escape hatch — `SetWindowTheme(tab_hwnd, L"", L"")` to drop the theme — does make `NM_CUSTOMDRAW` fire, but trades the themed Aero chrome for the classic flat look, which the user found jarring (this is the same Win11/UxTheme cooperation gap that bites our `WS_POPUP` dialogs in the first bullet of this section). The two viable Phase 5 paths: (a) full owner-draw via `TCS_OWNERDRAWFIXED` + `WM_DRAWITEM` — we paint every tab from scratch (text, icon, background, indicator) so the indicator is just one of our own paint operations; (b) buffered-paint via `BeginBufferedPaint` to overlay the indicator on top of the theme engine's output without disabling the theme. (a) is more code but gives full visual control (including dark mode); (b) preserves the theme but is fiddlier and Win10+ only. Worth attempting (a) alongside the `#32770` dialog migration since both involve trading themed-chrome convenience for full-paint control.
