...
- **`--perf` is specified in §8 but has never been implemented.** §8's verification column says cold start is "logged when `--perf` flag is set" and §7.3 lists "cold-start time measured against the §8 budget" as a re-tested item at every phase boundary — but no backend parses that flag and no timer exists. The 35–43 ms recorded for Phase 5 m1 came from ad-hoc instrumentation that was deleted with the branch it was written on, so it is not reproducible, and m3 could only measure exec-to-window-exists from outside the process instead (a different quantity: it includes spawn and dynamic linking, and excludes the first paint). The gate is therefore unenforceable as written. Implementing it is small — a flag on `codepp-app`, a `std::time::Instant` captured at the top of `main`, and one `tracing::info!` at first draw on each backend (`WM_PAINT` on Win32, the `draw` signal's first emission on GTK) — but it has to land on both backends together or the numbers stop being comparable across platforms, which is the whole point of a shared budget. Worth doing before the Phase 5 boundary, since that boundary is where §7.3 says the budget gets checked. **Keystroke latency has the same problem and is the more urgent half**: §8 budgets 5 ms p99 and DESIGN.md §8's own note says GTK's input delivery and cairo painting have never been profiled, so that constraint has never once been verified on Linux.
...
**Cold start.** Not directly comparable to m1's figure, because **the `--perf` flag this section specifies has never been implemented** — m1's 35–43 ms "to first draw" came from ad-hoc instrumentation that was not kept. What m3 could measure without it is time from `exec` to the toplevel window existing, timed out-of-process against the X server (so it includes process spawn and dynamic linking, which an in-app timer would exclude, and excludes the first paint, which m1's number included):

| Build | min | median | max |
| --- | --- | --- | --- |
| m2 (`0fda3db`) | 13.4 ms | 16.2 ms | 17.9 ms |
| m3, tab strip (`HEAD`) | 13.9 ms | 14.9 ms | 22.0 ms |

The two are indistinguishable, which is the question that mattered for this milestone. Reconciling with m1's number: m1's breakdown put first draw at +17.9 ms after widget construction, and 14.9 + ~18 ≈ 33 ms lands where m1 measured — consistent, but inferred rather than measured. **Implementing `--perf` on both backends is tracked in §7.4**, because "cold-start time measured against the §8 budget" is listed in §7.3 as a phase-boundary gate and cannot honestly be run without it.

**Not yet measured on GTK: keystroke latency.** The direct-call path is architecturally identical to Win32 — `ScintillaGTK::DirectFunction` is captured once by `EditorHandle::from_gtk_widget` — so calls *into* Scintilla cost the same. What differs is input delivery (GTK's `key-press-event` signal rather than `WM_CHAR`) and painting (cairo rather than GDI/Direct2D). Neither is on the direct-call path, and neither has been profiled. This is a gate item for Phase 5 m2 and needs the `--perf` frame-time log to exist on GTK first; do not assume parity until it is measured.
